...
- Added a top-level `--worktree` / `-w` flag that runs the session inside an isolated workspace, reusing the same user-chosen isolation primitive as subagent tasks: the `task.isolation.mode` setting drives the native PAL (CoW clone, snapshot, or recursive copy under `~/.omp/wt/`). Because the workspace is persistent and must stay a real, inspectable directory after exit, mount-only backends (overlayfs/projfs) are excluded — an explicit mount-only `task.isolation.mode` is rejected, and auto-resolution downgrades to rcopy rather than leaving a live mount behind. The session enters the workspace before any settings, plugin, or session work, so every file and shell tool is scoped to it without touching the active checkout. The name is optional — omit it for an auto-generated one (e.g. `swift-falcon`). The workspace is left in place on exit for manual inspection and merge; clean it up with `omp wt clear --all`, which unregisters rcopy git worktrees (a bare `rm -rf` would leave a stale `.git/worktrees` registration that breaks a same-name reuse). Re-running `--worktree <name>` for the same repo refuses instead of overwriting the existing workspace, so unmerged edits are never silently destroyed, and each workspace is tagged with a marker so `omp wt clear` keeps it (live, not a task-isolation leftover) unless `--all` is passed. The creation banner is written to stderr so `--mode json` stdout stays clean. Pull-request refs (`#123`) are redirected to `omp gh pr_checkout` ([#452](https://github.com/can1357/oh-my-pi/issues/452)).
