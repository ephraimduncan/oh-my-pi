...
- Daily upstream sync runs via `.github/workflows/sync-upstream.yml`. `AGENTS.md` is set to `merge=ours` in `.gitattributes`, so routine syncs never conflict on it; if you want upstream's `AGENTS.md` edits, reconcile by hand. Other conflicts make the run fail and write the conflicting files to the workflow run's job summary (issues are disabled on the fork). One-time local setup so manual merges honor the rule: `git config merge.ours.driver true`.
- A GitHub Release is published automatically on every push to `main` (`fork-auto-release.yml`) and after every successful upstream sync (`sync-upstream.yml` → `fork-release.yml`); releases are tag-only and never bump `package.json`. Cut a versioned release manually with `bun run release <version>`.
